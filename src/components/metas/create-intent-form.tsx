    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onCancel} className="text-xs text-pulso-text-secondary hover:text-pulso-text-muted transition-colors">
            Cancelar
          </button>
        </div>
        ...
        <div className="space-y-2">
          {INTENT_OPTIONS.map((option) => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-xl border border-pulso-border bg-pulso-elevated p-4 text-left transition-all hover:border-${option.color}-500/40 hover:bg-${option.color}-500/5`}
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-pulso-muted text-xl">
                {option.icon}
              </span>
              <div className="flex-1">
                <div className="text-sm font-medium">{option.label}</div>
                <div className="text-[10px] text-pulso-text-secondary">{option.description}</div>
              </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1 text-xs text-pulso-text-secondary hover:text-pulso-text-muted transition-colors"
        >
      ...
      <div className="space-y-3">
        <div>
          <label className="mb-1 block text-xs text-pulso-text-muted">Nome da meta</label>
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder={`Ex: ${selectedType!.label} este mes`}
            className="w-full rounded-lg border border-pulso-border bg-pulso-muted px-3 py-2 text-sm text-pulso-text placeholder-pulso-text-muted focus:border-emerald-500 focus:outline-none"
          />
        </div>

        <div>
          <label className="mb-1 block text-xs text-pulso-text-muted">
        ...
        <div>
          <label className="mb-1 block text-xs text-pulso-text-muted">Periodo</label>
              ...
              <button
                ...
                className={`flex-1 rounded-lg border px-3 py-2 text-xs transition-colors ${
                  periodType === p.value
                    ...
                    : 'border-pulso-border text-pulso-text-muted hover:border-pulso-primary/30'
                }`}
              >
                {p.label}
              </button>
        ...
        {periodType === 'custom' && (
          <div>
            <label className="mb-1 block text-xs text-pulso-text-muted">Data final</label>
            <input
              type="date"
              value={customEndDate}
        ...
        <div>
          <label className="mb-1 block text-xs text-pulso-text-muted">Descricao (opcional)</label>
          <input
            type="text"
            value={description}
      ...
      <button
        ...
        className="w-full rounded-lg bg-emerald-600 px-4 py-3 text-sm font-medium text-pulso-text transition-colors hover:bg-emerald-500 disabled:opacity-50"
      >
