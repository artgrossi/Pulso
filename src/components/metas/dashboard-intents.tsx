...
export function DashboardIntents() {
  ...
  if (isLoading) {
    return <div className="h-24 animate-pulse rounded-2xl border border-gray-800 bg-gray-900/60" />;
  }
  ...
  if (intents.length === 0) {
    return (
      <Link
        href="/metas"
        className="flex items-center gap-3 rounded-2xl border border-dashed border-gray-700 bg-gray-900/30 p-4 transition-all hover:border-emerald-500/40 hover:bg-emerald-500/5"
      >
        <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/10 text-xl">🎯</span>
        <div className="flex-1">
          <div className="text-sm font-medium text-gray-300">Defina uma meta</div>
          <div className="text-[10px] text-gray-500">Crie objetivos e acompanhe seu progresso</div>
        </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-medium uppercase tracking-wider text-gray-500">Metas Ativas</h3>
        ...
        {intents.slice(0, 3).map((intent) => {
          ...
          return (
            <Link
              key={intent.id}
              href="/metas"
              className="flex items-center gap-3 rounded-xl border border-gray-800 bg-gray-900/60 p-3 transition-all hover:border-gray-700 backdrop-blur-sm"
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-800 text-base">
                {icon}
              </span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-xs font-medium">{intent.title}</span>
                  <span className="shrink-0 text-[10px] text-gray-400">{intent.progress_percentage}%</span>
                </div>
