...
export function DiagnosisWizard() {
  ...
  if (showResult) {
    ...
    return (
      <div className="mx-auto w-full max-w-lg">
        <div className="rounded-2xl border border-pulso-border-subtle bg-pulso-elevated shadow-sm p-8 backdrop-blur-sm">
          <div className="mb-6 text-center">
            ...
            <p className="text-pulso-text-muted">{track.description}</p>
          </div>
  ...
  return (
    <div className="mx-auto w-full max-w-lg">
      {/* Progress bar */}
      <div className="mb-8">
        <div className="mb-2 flex items-center justify-between text-xs text-pulso-text-secondary">
          <span>Pergunta {currentStep + 1} de {totalSteps}</span>
          <span>{Math.round(progress)}%</span>
        </div>
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-pulso-muted">
          <div
      ...
      <div className="rounded-2xl border border-pulso-border-subtle bg-pulso-elevated shadow-sm p-8 backdrop-blur-sm">
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full rounded-xl border border-pulso-border bg-pulso-muted/50 px-6 py-4 text-left text-sm font-medium text-white transition-all hover:border-emerald-500/50 hover:bg-emerald-500/10"
          >
            {question.yes_label}
          </button>
          <button
        ...
        {currentStep > 0 && (
          <button
            onClick={handleBack}
            className="mt-6 w-full text-center text-xs text-pulso-text-secondary transition-colors hover:text-pulso-text-secondary"
          >
